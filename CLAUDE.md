...
종류만 적고 실제 값은 비공개 저장소 `fos-home-infra` 의 목록이 갖는다.

이 규칙은 검사 스크립트에도 똑같이 걸린다.
`scripts/verify-public-infra-boundary.sh` 와 그 회귀 검사가 값 목록을 그대로 담고 있어 실제로 한 번 새어 나갔다.
검사기는 값이 아니라 모양만 갖고, 값 목록은 `fos-home-infra` 에서 읽는다.

대신 이렇게 쓴다.

- 무엇을 확인해야 하는지만 적고, 실행 방법은 `fos-home-infra` 를 가리킨다
- 측정한 **결과 수치**는 적어도 된다. 그것을 얻은 명령을 적지 않는 것이다
- 조사 기록은 공개 저장소 밖이나 `private/` 에 둔다

제품과 도구의 공개된 이름은 값이 아니다. 그대로 쓴다.
...
값 목록을 읽지 못하면 검사는 형태 패턴만 보고 그 사실을 알린다.
그때는 통과해도 값 층을 확인하지 못한 것이다.

밖으로 열어 둔 공개 주소처럼 감출 것이 없는 자리는 그 값 목록의 허용 층에 적어 검사에서 뺀다.
허용은 줄 단위라 같은 규칙에 걸리는 다른 줄은 그대로 걸린다.
