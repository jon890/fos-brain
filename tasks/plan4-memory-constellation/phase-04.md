...
브라우저 console error가 0개이며 홈에서만 3D chunk 요청이 생기는지 network 기록으로 확인한다.

### 3. 모바일과 실패 폴백 검사

390px 너비에서 필터 서랍과 상세 시트의 열기·닫기, 가로 overflow 0, 터치 목표와 키보드 focus를 검사한다.
reduced-motion 환경에서 궤도와 자동 카메라 이동이 비활성화되는 상태를 확인한다.
콘텐츠 색인 요청을 실패시켜 오류 안내, 다시 시도, 문서 결과 목록이 남는지 검사한다.
...
실제 브라우저 검사는 데스크톱과 모바일 screenshot, 접근성 snapshot, console·network 출력 경로를 실행 보고에 남겨야 한다.
