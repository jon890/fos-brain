...
**범위 외**: wiki 본문과 외부 인증 계층 설정은 변경하지 않는다.
...
홈의 `body`에는 서버 렌더 단계부터 `memory-atlas-page` class를 지정하고 SPA 이탈 cleanup에서도 같은 경계를 유지한다.
...
| `quartz/quartz/components/renderPage.tsx` | 홈과 문서 셸을 서버 렌더 단계에서 구분 |
