...
login 전 public session, private·질문 `401`, login 뒤 protected JSON·질문 성공, logout 뒤 재차단을 HTTP 수준에서 검사한다.
...
| `quartz/scripts/verify-memory-atlas-browser.sh` | 권한 전환 browser 회귀 진입점 |
...
완료 보고에는 public·admin 상태별 HTTP 결과, 두 viewport browser 결과, image 실행 사용자와 public 누출 검사 결과를 포함한다.
운영 전환은 이 phase의 성공 commit을 private 인프라 계획에 입력한 뒤 시작한다.
