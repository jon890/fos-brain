...
배치를 다시 계산하는 경로는 배율을 유지한 채 이동만 새 중심에 맞춘다. 처음 상태로 되돌리지 않는다.
지역 배치가 선택 노드를 장면 중앙에 두므로, 배율 `k` 일 때 이동은 `(w/2 - k*w/2, h/2 - k*h/2)` 다.
`select` 는 slug 가 있을 때와 `undefined` 일 때를 가리지 않고 이 규칙을 받는다.
`memoryAtlasController.ts` 의 `selectNode` 는 slug 유무와 무관하게 `runtimeLifecycle.select(slug)` 를 부르므로 두 경로가 같은 동작을 받는다.

`update` 도 배치에 영향을 주는 입력(데이터, 의미 관계, `layout`, `spacing`, `selectedSlug`)이 바뀌면 같은 규칙을 적용한다.
`refresh` 가 필터와 배치 변경을 `update` 로 흘리므로, 여기를 빼면 같은 상황에서 두 경로의 동작이 갈린다.
`colorBy` 와 `labels` 처럼 좌표를 바꾸지 않는 변경은 이동을 건드리지 않는다.

### 2. 배율 조작 그룹

`memoryAtlasView.tsx` 의 `.memory-atlas__topbar` 에 `.memory-atlas__viewport-controls` 그룹을 두고 버튼 셋을 담는다.
라벨은 `축소`, `확대`, `전체 보기` 이며 모두 `type="button"` 이다.
`data-testid` 는 각각 `memory-atlas-zoom-out`, `memory-atlas-zoom-in`, `memory-atlas-reset-viewport` 로 controller 가 찾을 수 있게 한다.
이 저장소는 단일 컨트롤을 `data-testid="memory-atlas-*"` 로 찾고 `data-memory-atlas-*` 는 반복 요소에 쓴다.

그룹을 rail 이 아니라 topbar 에 두는 이유는 800px 이하에서 `.memory-atlas__rail` 이 `translateX(-105%)` 로 화면 밖에 있기 때문이다.
터치에는 wheel 이 없고 `.memory-atlas-2d` 는 `touch-action: none` 이라, 버튼이 없으면 좁은 화면에 배율을 바꿀 수단이 남지 않는다.

설명은 `title` 로 준다. `aria-label` 로 주면 접근 가능한 이름이 시각 라벨을 덮어 음성 조작이 `전체 보기` 로 버튼에 닿지 못한다(WCAG 2.5.3).

`zoomBy(factor)` 를 handle 의 선택 메서드로 더한다. 2D 는 컨테이너 중앙을 기준점으로 삼아 휠과 같은 배율 계산을 쓴다.
버튼 한 번이 바꾸는 비율 `MEMORY_ATLAS_2D_BUTTON_SCALE_STEP` 은 controller 와 runtime 이 함께 쓰므로 `memoryAtlasRuntimeTypes.ts` 에 둔다.
2D 번들을 controller 로 끌어들이지 않기 위해서다.
...
3D 모드에서는 이 그룹을 통째로 숨긴다. `syncControls` 가 `state.mode` 를 볼 때 함께 처리한다.
3D 는 `resetViewport` 와 `zoomBy` 를 구현하지 않으므로 버튼을 남기면 눌러도 반응이 없는 컨트롤이 된다.
...
- `확대` 와 `축소` 를 누르면 포인터 없이도 `transform` 의 `scale` 이 바뀐다.
- 배율을 올린 채 노드를 고르면 그 배율이 유지되고 선택 노드가 화면 중앙에 온다.
- 3D 모드로 바꾸면 배율 조작 그룹이 숨는다.
...
- SSR 마크업의 topbar 안에 배율 조작 버튼 셋이 나오고, `전체 보기` 는 `type="button"` 과 `title` 을 가지며 `aria-label` 이 없다. `전체 지도로` 와도 별개로 남는다.
- `centerMemoryAtlas2dViewport` 가 배율을 유지한 채 장면 중앙을 화면 중앙에 맞춘다.
- `scaleMemoryAtlas2dViewport` 가 기준점을 고정한 채 배율만 바꾸고 `0.4`, `4` 경계에서 멈춘다.
- `affectsMemoryAtlas2dLayout` 이 좌표를 바꾸는 입력만 참으로 판정한다.

`select` 와 `select(undefined)` 가 이동을 새 중심에 맞추는지는 단위 검사로 확인하지 않는다.
...
- 선택에서 배율까지 되돌리지 않는 이유는 확대한 채 여러 노드를 훑는 흐름이 끊기기 때문이다. 되돌릴 수단은 `전체 보기` 가 따로 맡는다.
- `update` 에도 같은 규칙을 적용하는 이유는 `refresh` 가 필터와 배치 변경을 그 경로로 흘리기 때문이다. 한쪽만 맞추면 같은 상황에서 지도가 다르게 움직인다.
