...
`.agents/plugin/fos-brain/scripts/setup-check.cjs`가 `/Users/nhn/.local/bin-pinned/qmd`만 사용하게 한다.
고정 wrapper가 없으면 명시적인 경고를 남기고 컬렉션 작업을 생략한다.
`bun.lock` 생성, 전역 자동 설치, PATH의 일반 `qmd` 실행은 하지 않는다.
...
고정 wrapper가 없을 때도 경로를 밝힌 진단과 0이 아닌 종료 코드를 반환한다.
...
! rg -n 'install -g|bun.*install|npm.*install|return "qmd"' .agents/plugin/fos-brain/scripts/setup-check.cjs .agents/plugin/fos-brain/scripts/retrieval-benchmark.cjs
