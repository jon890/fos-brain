...
얇은 shell 진입점과 `verify-memory-atlas-browser.mjs`가 `~/.claude/scripts/browser-driver`의 `doctor`, `open`, `waitjs`, `js`, `shot`, `console`, `errors`, `worktree`, `close`만 사용하게 한다.
...
browser assertion은 JavaScript 함수와 이름을 Node 모듈에 두고 shell heredoc으로 만들지 않는다.
성공과 실패 경로 모두에서 탭, 임시 server와 화면 산출물을 정리한다.

### 2. 2D 전체·지역 관계 시나리오

1440×1000과 390×844에서 기본 mode가 2D이고 문서와 보이는 조작 요소의 `scrollWidth`가 `clientWidth`를 넘지 않는지 검사한다.
...
키보드 `Tab`, `Enter`, `Space`, `Escape`만으로 같은 흐름을 완료하고 움직임 줄이기 환경에서는 중간 animation이 없는지 검사한다.

### 3. mode·실패·privacy 회귀

2D에서 선택과 filter를 만든 뒤 3D로 전환해 같은 선택이 유지되고 3D runtime이 그때 처음 요청되는지 검사한다.
다시 2D로 돌아오면 3D canvas와 renderer 자원이 제거되고 2D 노드만 남아야 한다.
SPA 문서 이동과 홈 복귀 뒤 mode와 선택을 복원하고 선택 해제 뒤 이전 강조가 되살아나지 않는지 검사한다.

browser 안에서 fetch를 통제한 뒤 `nav` 재초기화를 실행해 의미 관계 파일 오류는 link·tag 축소 상태로, 콘텐츠 색인 오류는 서버 렌더 문서 목록과 retry 상태로 나타나는지 검사한다.
public fixture에 private 의미 edge와 시작점 후보를 넣어도 DOM, 상태 문구, network 응답을 정제한 화면에 `_private/` slug와 private 제목이 나타나지 않아야 한다.
