...
- **type 분포**: chore 가 가장 많고 docs·feat·fix·refactor 순. 문서·잡무 커밋 비중이 크다(정원 가꾸기형).
- **scope 의미**: 도메인명(`feat(expense)`), 모듈(`fix(resolvers)`), 또는 task 식별자(`docs(plan024)`), OCR-docs 는 경로(`docs(ko/api-guide-v2.1)`).
- **atomic 커밋**: 한 커밋 = 하나의 논리 변경. 기능·문서·스킬 변경을 분리.
- **추적성**: 제목에 PR 번호(`(#171)`)·이슈(`(closes #NN)`)·`plan{N}` 식별자 부착.
...
- **브랜치 분리**: 계획·docs 는 `plan/{N}` 또는 `tasks/`, 구현은 `feat/plan{N}`. main 직접 push 금지.
