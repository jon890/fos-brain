...
- reward 부재가 곧 자동화 불가의 벽이다.
  ...
  - 직접 만든 `docs_score`·`brain_score`(위반 수 가중 감점)처럼 reward 가 있으면 게이트가 작동한다.
...
- 대화형·사람 확인(미리보기 → confirm) 중심 워크플로우는 자동 채점 루프와 근본적으로 안 맞으므로, 도구가 아니라 규율(편집 예산·검증 게이트)만 빌린다.
