---
name: brain-delete
description: 개인 지식 기반(brain, ~/personal/fos-brain) 에서 wiki 페이지(개념·주제·개체)를 안전하게 제거하거나 보관 처리한다. 단순 파일 삭제가 아니라 백링크·INDEX·log·네임스페이스 간 링크를 함께 정리해 깨진 링크와 고아 참조를 남기지 않는다. raw 원본은 기본 보존. "brain delete", "brain 삭제", "brain 에서 제거", "지식 삭제", "이 개념 지워줘", "wiki 삭제", "페이지 삭제", "노트 삭제", "brain 에서 빼줘", "이 지식 제거" 같은 요청 시 사용.
---

# brain-delete

brain 에서 더 이상 필요 없거나 잘못 들어온 wiki 페이지를 안전하게 떼어낸다.
brain은 백링크·INDEX·log·Sources·네임스페이스 간 링크로 연결돼 있어, 파일만 지우면 깨진 링크와 고아 참조가 남는다.
...
- **백링크** — 대상을 `[[...]]`로 가리키는 다른 페이지 목록이다. 같은 네임스페이스의 링크와 비공개에서 공개로 향하는 링크를 포함한다.
  ```bash
  # 대상 파일명을 가리키는 페이지 찾기 (네임스페이스 루트에서)
  rg -l "\[\[.*<대상-파일명>.*\]\]" <ns>/wiki/
  ```
- **INDEX 등록 항목** — `<ns>/wiki/INDEX.md` 에 대상이 등록돼 있는지.
- **대상이 인용한 raw Sources** — 대상 페이지 하단 "Sources" 의 `raw/` 경로(삭제하지 않음, 보고만).
- **네임스페이스 간 링크 점검** — 대상이 public인데 비공개에서 폴더 경로 없이 파일명만 쓴 링크로 참조 중이면 함께 보고한다. 페이지를 삭제하면 이 참조도 끊긴다.
