...
8. **stale 마커 탐지** — 페이지 본문에서 미완료를 뜻하는 표현을 grep 으로 찾는다.
   ...
   - 이건 발견만 한다(구조 검사). "지금 실제로 완료됐는가"는 사람 판단이 필요해 12번 품질 축에서 다룬다.
9. **페이지 간 모순(의미 검사)** — 같은 사실·수치에 두 페이지가 다른 주장. 출처(raw)가 다르면 모순이 아닐 수 있음.
10. **누락된 교차 참조 제안(의미 검사)** — 본문에서 다른 개념을 언급하는데 `[[...]]` 가 없는 경우 후보 제안. 자동 추가 금지.
11. **공개/비공개 링크 누출(보안)** — 공개(public) 페이지가 `private/` 또는 `work/` 를 링크하는지. 발견 시 즉시 보고(유출 위험). 비공개 → 공개는 정상.
12. **품질 축 (Quality Loop, 명시 요청 시)** — 기존 페이지의 *내용 품질*을 재판단한다. 구조 검사(1~11)와 별개.
    - **정확성 재검토** — 출처(raw)와 대조해 주장이 여전히 맞는지. 사용자 정정 여지 큰 주관 판단 플래그.
    - **미검증·신뢰도** — 2차 출처(자동자막·에이전트 생성물)나 근거 없는 단정 표시.
    - **stale** — 8번이 찾은 미완료 마커 페이지를 실제로 검토한다. 최신 사실과 대조해 이미 완료됐으면 "완료 처리" 로, 여전히 유효하면 "유지" 로 분류한다. `brain-curate` 의 dedup 단계가 어떤 후보를 "reinforce" 로 판정했는지도 참고한다 — 그 판정 자체가 종종 "기존 페이지가 최신 상태를 못 따라갔다"는 신호다(실제 사례: `ocr-requestid-trace.md` 의 "모델 서버 측 (남은 작업)" 이 2026-07-01 brain-curate 로 완료 사실이 확인돼 갱신됨).
...
1. **검사만 먼저(read-only)** — 네임스페이스별로 위 11개 구조 항목을 훑어 발견 사항을 표로 보고.
...
12개 항목 중 *객관 채점 가능한* 구조 항목만 점수로 환산해 수정 전후를 비교한다.
중복·stale마커·모순·교차참조·품질축(3·8·9·10·12)은 주관 판단이거나 발견 전용이라 점수에서 제외한다.
