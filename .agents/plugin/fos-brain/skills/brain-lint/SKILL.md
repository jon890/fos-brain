...
11. **품질 축 (Quality Loop, 명시 요청 시)** — 기존 페이지의 *내용 품질*을 재판단한다. 구조 검사(1~10)와 별개.
    ...
    - **stale** — 7번이 찾은 미완료 마커 페이지를 실제로 검토한다. 최신 사실과 대조해 이미 완료됐으면 "완료 처리" 로, 여전히 유효하면 "유지" 로 분류한다. `brain-curate` 의 dedup 단계가 어떤 후보를 "reinforce" 로 판정했는지도 참고한다 — 그 판정 자체가 종종 "기존 페이지가 최신 상태를 못 따라갔다"는 신호다(실제 사례: `ocr-requestid-trace.md` 의 "모델 서버 측 (남은 작업)" 이 2026-07-01 brain-curate 로 완료 사실이 확인돼 갱신됨).
