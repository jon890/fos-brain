...
`~/personal/fos-brain` — 네임스페이스별(public·private)로 점검한다.
...
10. **공개/비공개 링크 누출(보안)** — 공개(public) 페이지가 `private/`를 링크하는지. 폐기된 `work/` 경로가 다시 나타나도 함께 보고한다. 비공개 → 공개는 정상.
...
| 채점 축 | 가중 | 근거 |
|---|---|---|
| visibility_leak | 10 | public 이 private 또는 폐기된 work 경로를 링크 (유출, 보안 최고) |
