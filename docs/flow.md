...
9. 보호 release 전환이 성공하면 Jenkins가 별도 단계에서 Hermes의 qmd 컬렉션을 확인하고 `update`, `embed`를 실행한다.
10. qmd 갱신은 전용 잠금으로 직렬화하고 public wiki, public raw, private wiki만 정해진 컬렉션에 반영한다.
11. 보호 빌드가 실패하면 직전 `current`를 유지하며 운영자는 Jenkins에서 같은 작업을 수동으로 다시 실행할 수 있다.
12. qmd 갱신만 실패하면 활성 release는 유지하고 Jenkins 작업을 불안정 상태로 표시해 재시도 대상을 구분한다.

두 저장소 push가 겹치면 뒤 작업이 잠금을 기다린다.
잠금을 얻은 작업은 두 저장소의 최신 `main`을 다시 읽으므로 중간 push를 하나의 최종 산출물로 합칠 수 있다.
Quartz 빌드는 웹 검색 색인을 갱신하고, 뒤따르는 별도 단계가 Hermes의 qmd 임베딩을 증분 갱신한다.
