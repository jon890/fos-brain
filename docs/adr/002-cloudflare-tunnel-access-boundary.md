...
- **결정**: hosting.kr은 등록기관으로 유지하고 Cloudflare full DNS setup으로 권한 DNS만 위임한다.
  ...
  공개 호스트 이름 8개는 호스트 이름과 일치하는 NPM 인증서를 검증하며 HTTPS 원본으로 전달한다.
  `brain`의 ACME challenge 경로만 인증서 갱신을 위해 별도 Bypass로 둔다.
  전환 검증 뒤 NPM의 호스트 80·81·443을 loopback으로 제한하고 Cloudflare DNSSEC와 등록기관 DS를 연결한다.
- **맥락**: 결정 당시 홈서버는 공인 80·81·443을 NPM에 직접 노출했으며 Grafana, Jenkins, NPM과 새 brain은 사용자 인증 경계가 필요했다.
...
- **트레이드오프**: Cloudflare와 단일 Tunnel 장애가 모든 HTTP 서비스의 공통 장애점이 된다.
  ...
  인증서 자동 갱신을 위해 `brain`의 정확한 ACME challenge 경로는 공개되지만 일반 콘텐츠 경로에는 Allow 정책을 유지한다.
