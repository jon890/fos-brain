...
- **결정**: 내부 wiki의 기존 frontmatter와 폴더 경로 없이 파일명만 쓴 링크를 유지하고, OKF v0.2와 겹치는 의미 필드를 점진적으로 추가한다.
  외부 교환은 public 전용 내보내기에서 표준 Markdown 링크와 OKF 묶음으로 변환한다.
  묶음의 index와 log는 OKF 예약 문서로 분리하고, 일반 지식 문서에만 메타데이터를 보완한다.
- **맥락**: fos-brain의 공개·비공개 병합 Quartz는 폴더 경로 없이 파일명만 쓴 링크에 의존한다.
