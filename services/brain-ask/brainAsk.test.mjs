...
test("parseQmdUri allows wiki collections and rejects traversal, raw, absolute and symlink escape", async () => {
  ...
  try {
    ...
    assert.equal(parseQmdUri("qmd://brain-private/entities/style.md", {
      publicWikiRoot: brain.publicWiki,
      privateWikiRoot: brain.privateWiki,
    }).href, "/_private/entities/style");
    assert.throws(() => parseQmdUri("qmd://brain-raw/source.md", { publicWikiRoot: brain.publicWiki }), /not allowed/);
    assert.throws(() => parseQmdUri("qmd://brain-wiki/../secret.md", { publicWikiRoot: brain.publicWiki }), /escapes/);
    assert.throws(() => parseQmdUri("qmd://brain-wiki/%2Fetc/passwd", { publicWikiRoot: brain.publicWiki }), /relative/);
    ...
    if (fsSync.symlinkSync) {
      ...
      assert.throws(() => parseQmdUri("qmd://brain-wiki/concepts/outside.md", {
        publicWikiRoot: brain.publicWiki,
        privateWikiRoot: brain.privateWiki,
      }), /outside/);
    }
...
test("selectEvidence preserves qmd order and caps file and total context size", async () => {
  ...
  try {
    ...
    assert.deepEqual(sources.map((source) => source.title), ["Doc 0", "Doc 1", "Doc 2", "Doc 3"]);
...
test("server handles answer, empty search, bad input, busy, upstream errors and safe logs", async () => {
  ...
  const qmd = await startJsonServer(async (req, res) => {
    ...
    res.end(JSON.stringify({
      results: qmdMode === "empty" ? [] : [
        { uri: "qmd://brain-wiki/concepts/agent.md", title: "Agent", score: 0.9, excerpt: "public excerpt" },
      ],
    }));
  });
  ...
  try {
    let response = await postJson(server.address().port, { question: "내 작업 방식?" });
    ...
    const [first, second] = await Promise.all([
      postJson(server.address().port, { question: "느린 질문" }),
      postJson(server.address().port, { question: "동시 질문" }),
    ]);
    ...
    qmdMode = "slow";
    response = await postJson(server.address().port, { question: "qmd 시간 초과" });
    assert.equal(response.status, 502);
    assert.equal(response.body.error.code, "retrieval_unavailable");
    ...
    modelMode = "slow";
    response = await postJson(server.address().port, { question: "모델 시간 초과" });
