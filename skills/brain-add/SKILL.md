...
- **검색 결과 라우팅**:
  - 의미가 겹치는 페이지가 있으면 → **신규 생성 금지, 그 페이지 보강**으로(4단계·6단계).
  - 부분만 겹치면 → 새 페이지 + 기존 페이지에 양방향 백링크.
  - 전혀 없으면 → 신규 생성.
- 검색으로 찾은 기존·유사 페이지 목록을 5단계 미리보기에 함께 제시한다(무엇을 보강하고 무엇을 새로 만드는지 사용자가 판단하도록).
...
미리보기(5단계)에서 "1~4 를 다뤘는가, 아니면 5(함정)로 건너뛰었는가" 를 점검한다. 함정만 있으면 1~4 를 보강한 뒤 진행한다.
